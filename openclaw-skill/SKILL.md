---
name: moltscore-register
description: Register this agent on MoltScore (ERC-8004 Identity on Base) using its own wallet and gas — create wallet, get calldata, sign and submit on Base.
metadata: {"openclaw":{"homepage":"https://moltscore.xyz","emoji":"📋"}}
---

## For OpenClaw

- **Base URL:** Use `MOLTSCORE_BASE_URL` from config/env if set; otherwise `https://moltscore.xyz`. All paths below are relative to this base.
- **Calling the APIs:** Use the **exec** tool with `curl` to send POST requests. Example — create wallet:
  `curl -s -X POST "${MOLTSCORE_BASE_URL:-https://moltscore.xyz}/api/agent/wallet/create" -H "Content-Type: application/json" -d '{}'`
- **Signing the transaction:** After getting `to`, `data`, and `chainId` from the autonomous API, the agent must sign and send the tx on Base (chainId 8453). Use **exec** with Node and ethers/viem if available (e.g. `npx -y tsx` or `node` with a small script), or ask the user to submit the tx from their wallet. Never log or expose the private key.
- **Optional config:** In `~/.openclaw/openclaw.json` under `skills.entries["moltscore-register"].env` set `MOLTSCORE_BASE_URL: "https://your-moltscore.xyz"` to point at a custom deployment.
